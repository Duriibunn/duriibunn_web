...
export default function TripDetailPage() {
  ...
  const handleEdit = () => {
    // 현재 여행 데이터를 가져와서 DailySchedulePage로 전달
    const currentTripData = localStorage.getItem('currentTrip');
    if (currentTripData) {
      const tripData = JSON.parse(currentTripData);
      ...
      showToast('일정 데이터를 불러올 수 없습니다', 'error');
    }
  };
  ...
  const handleDelete = () => {
    ...
    // localStorage에서 currentTrip 삭제
    localStorage.removeItem('currentTrip');
    
    // myTrips에서도 해당 여행 삭제 (ID 기반)
    const myTripsData = localStorage.getItem('myTrips');
    if (myTripsData) {
      const trips = JSON.parse(myTripsData);
      const currentTripId = currentItinerary?.id;
      const updatedTrips = trips.filter((t: { id: string }) => t.id !== currentTripId);
